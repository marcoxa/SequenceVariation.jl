...
### Edit.jl
###
### Code to handle "edits" of a sequence.
...
An edit of either `Substitution{T}`, `Insertion{S}` or `Deletion` at a
position.
...
function Base.parse(::Type{T}, s::AbstractString) where
    {T <: Edit{Se, Sy}} where {Se,S y}
    return parse(T, String(s))
end


function Base.parse(::Type{<:Edit{Se,Sy}},
                    s::Union{String,SubString{String}}) where {Se, Sy} 
    ## Either "Δ1-2", "11T" or "G16C"
    if (m = match(r"^Δ(\d+)-(\d+)$", s); m) !== nothing
        pos = parse(UInt, m[1])
        stop = parse(UInt, m[2])
        stop ≥ pos ||
            throw(ArgumentError("Non-positive deletion length: \"" * s * "\""))
...
Returns the underlying [`Substitution`](@ref), [`Insertion`](@ref), or
[`Deletion`](@ref) of `e`.
"""
...
function _lendiff(edit::Edit)
    x = _mutation(edit)
    ## Each edit type has logic for its length, we just need to know
