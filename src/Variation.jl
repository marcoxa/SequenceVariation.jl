...
function Variation{S, T}(ref::S, e::Edit{S, T}) where
...
function Haplotype(ref::S, vars::Vector{Variation{S, T}}) where
    {S <: BioSequence, T <: BioSymbol}
    edits = _edit.(vars)
...
Gets the underlying [`Substitution`](@ref), [`Insertion`](@ref), or
[`Deletion`](@ref) of `v`.
"""
...
function Base.isless(x::Variation, y::Variation)
    reference(x) == reference(y) ||
        error("Variations cannot be compared if their reference "
...
Validate `v`. `v` is invalid if its operation is out of bounds.
"""
...
Convert the difference in `var` to a new reference sequence based upon
`aln`. `aln` is the alignment of the old reference (`aln.b`) and the
new reference sequence (`aln.seq`).

Returns the new [`Variation`](@ref).
"""
function translate(var::Variation{S, T},
    ...
    ## Special case: Insertions may have a pos of 0, which cannot be
    ## mapped to the seq using ref2seq
    ...
    if kind isa Substitution
        ...
        op in (BA.OP_MATCH, BA.OP_SEQ_MATCH, BA.OP_SEQ_MISMATCH) ||
            return nothing
    ...
    elseif kind isa Deletion
        ...
        ## If it's a deletion, return nothing if the deleted part is
        ## already missing from the new reference.
        ...
        if op in (BA.OP_MATCH, BA.OP_SEQ_MATCH, BA.OP_SEQ_MISMATCH)
            ...
            if pos != lastindex(ref) &&
               first(BA.ref2seq(aln, pos + 1)) != seqpos + 1
                return Inapplicable()
            else
            ...
            ## Alternatively, it can map to a deletion. In that case,
            ## it become really tricky to talk about the "same"
            ## insertion.
        else
            return Inapplicable()
        end
...
Converts the [`Edit`](@ref)s of `h` into a vector of
[`Variation`](@ref)s.
"""
...
Get the reference bases of `v`. Note that for deletions, `refbases`
also returns the base _before_ the deletion in accordance with the
`REF` field of the
...
Get the alternate bases of `v`. Note that for insertions, `altbases`
also returns the base _before_ the insertion in accordance with the
`ALT` field of the
[VCF v4 specification](https://samtools.github.io/hts-specs/VCFv4.3.pdf).
"""
